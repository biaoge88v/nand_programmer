...
QVariant SpiNorDbTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case SpiNorDb::CHIP_PARAM_ADDR_BYTES:
        return (uint)chipDb->getChipParam(index.row(),
            SpiNorInfo::CHIP_PARAM_ADDR_BYTES);
    case SpiNorDb::CHIP_PARAM_ENTER_4BYTE:
        chipDb->getHexStringFromParam(chipDb->getChipParam(index.row(),
            SpiNorInfo::CHIP_PARAM_ENTER_4BYTE), paramStr);
        return paramStr;
    case SpiNorDb::CHIP_PARAM_EXIT_4BYTE:
        chipDb->getHexStringFromParam(chipDb->getChipParam(index.row(),
            SpiNorInfo::CHIP_PARAM_EXIT_4BYTE), paramStr);
        return paramStr;
    ...
    Qt::Orientation orientation, int role) const
{
    if (role == Qt::DisplayRole && orientation == Qt::Horizontal)
    {
        ...
        case SpiNorDb::CHIP_PARAM_ADDR_BYTES: return tr("Addr bytes");
        case SpiNorDb::CHIP_PARAM_ENTER_4BYTE: return tr("Enter 4-byte com.");
        case SpiNorDb::CHIP_PARAM_EXIT_4BYTE: return tr("Exit 4-byte com.");
    ...
    if (role == Qt::ToolTipRole)
    {
        ...
        case SpiNorDb::CHIP_PARAM_ADDR_BYTES:
            return tr("Address bytes (3 or 4)");
        case SpiNorDb::CHIP_PARAM_ENTER_4BYTE:
            return tr("Command to enter 4-byte address mode ");
        case SpiNorDb::CHIP_PARAM_EXIT_4BYTE:
            return tr("Command to exit 4-byte address mode ");
        case SpiNorDb::CHIP_PARAM_FREQ:
    ...
    const QVariant &value, int role)
{
    ...
    case SpiNorDb::CHIP_PARAM_ADDR_BYTES:
        ...
        if (chipDb->getParamFromString(value.toString(), paramVal))
            return false;
        if (!chipDb->isParamValid(paramVal, 3, 4)) // 地址字节数只能是3或4
            return false;
        chipDb->setChipParam(index.row(), SpiNorInfo::CHIP_PARAM_ADDR_BYTES, paramVal);
        return true;
    case SpiNorDb::CHIP_PARAM_ENTER_4BYTE:
        // 解析十六进制命令，验证范围0x00-0xFF
        if (chipDb->getParamFromHexString(value.toString(), paramVal))
            return false;
        if (!chipDb->isParamValid(paramVal, 0x00, 0xFF))
            return false;
        chipDb->setChipParam(index.row(), SpiNorInfo::CHIP_PARAM_ENTER_4BYTE, paramVal);
        return true;
    case SpiNorDb::CHIP_PARAM_EXIT_4BYTE:
        // 解析十六进制命令，验证范围0x00-0xFF
        if (chipDb->getParamFromHexString(value.toString(), paramVal))
        ...
        if (!chipDb->isParamValid(paramVal, 0x00, 0xFF))
            return false;
        chipDb->setChipParam(index.row(), SpiNorInfo::CHIP_PARAM_EXIT_4BYTE, paramVal);
        return true;
    case SpiNorDb::CHIP_PARAM_FREQ:
        if (chipDb->getParamFromString(value.toString(), paramVal))
            return false;
